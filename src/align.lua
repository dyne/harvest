-- This code was ripped from Rosetta's stone
...
local function readInput(pStr)
    for line in pStr:gmatch("([^\n]+)[\n]-") do  -- read until '\n' character
        ...
        for word in line:gmatch("[^,]+") do      -- read non '$' character
            colCount = colCount + 1
			-- 6 is the minimum column width for padding
            tColLen[colCount] = math.max((tColLen[colCount] or 6), #word)   -- store column length
...
--main interface
--------------------------------------------------[]
function alignColumn(pStr, pAlignment)
--------------------------------------------------[]
